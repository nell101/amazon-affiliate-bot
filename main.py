...
class AmazonAffiliateBlogBot:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if 'access_token' in token_data:
                            ...
                            logger.warning("⚠️ No access_token in OAuth response")
                    else:
                        logger.warning(f"⚠️ OAuth refresh failed: {response.status_code} - {response.text}")
                except requests.exceptions.RequestException as e:
                    logger.warning(f"⚠️ OAuth refresh network error: {e}")
                except Exception as e:
                    logger.warning(f"⚠️ OAuth refresh error: {e}")
            
            # Fallback: try using refresh token directly if it looks like an access token
            if self.refresh_token:
                if self.refresh_token.startswith('ya29.'):
                    logger.info("✅ Using token directly (appears to be access token)")
                    self.access_token = self.refresh_token
                    self.token_expires_at = current_time + 3600
                    return self.access_token
                else:
                    logger.warning("⚠️ Token doesn't appear to be access token format")
    ...
    def generate_seo_content(self, product):
        """Generate SEO-optimized content using Google Gemini with retry logic"""
        for attempt in range(self.max_retries):
            try:
...
Format as JSON: {{"title": "...", "meta_description": "...", "content": "..."}}"""
                ...
                if response.status_code == 200:
                    ...
                    logger.error(f"❌ Gemini API error (attempt {attempt + 1}/{self.max_retries}): {response.status_code} - {response.text}")
                    if attempt < self.max_retries - 1:
                        time.sleep(self.retry_delay)
                        continue
